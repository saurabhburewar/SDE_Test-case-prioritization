...
class TPListWiseDQNAgent:

    def train_agent(self, env: gym.Env, steps: int, path_to_save_agent: None, base_model=None):
        env.reset()
        if not base_model:
    ...
    #@timeout(500)
    def test_agent(self, env: gym.Env, model_path: str, model):
        agent_actions = []
        ...
        if model:
            ...
            while True:
                ...
                if agent_actions.count(action) == 0:
