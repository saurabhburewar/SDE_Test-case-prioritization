...
#    -- fix end cycle issue
    ...
    for i in range(start_cycle, end_cycle - 1):
        ...
        if first_round:
            ...
            tp_agent.learn(total_timesteps=steps,
                           reset_num_timesteps=True, callback=callback_class)
            training_end_time = datetime.now()
            first_round = False
        else:
            tp_agent = TPAgentUtil.load_model(
                algo=algo, env=env, path=previous_model_path+".zip")
            training_start_time = datetime.now()
            tp_agent.learn(total_timesteps=steps,
                           reset_num_timesteps=True, callback=callback_class)
            training_end_time = datetime.now()
        ...
        test_case_vector = TPAgentUtil.test_agent(
            env=env_test, algo=algo, model_path=model_save_path+".zip", mode=mode)
...
if __name__ == '__main__':
    ...
    mode = 'pairwise'  # 'pointwise' 'listwise'
    algo = 'dqn'
    dataset_type = 'simple'  # 'enriched'
    episodes = 1

    supported_formalization = ['PAIRWISE',
                               'POINTWISE', 'LISTWISE', 'LISTWISE2']
    supported_algo = ['DQN', 'PPO2', "A2C", "ACKTR", "DDPG",
                      "ACER", "GAIL", "HER", "PPO1", "SAC", "TD3", "TRPO"]
    assert supported_formalization.count(
        mode.upper()) == 1, "The formalization mode is not set correctly"
    assert supported_algo.count(
        algo.upper()) == 1, "The formalization mode is not set correctly"

    conf = Config()
    conf.train_data = conf.train_data
    conf.dataset_name = Path(conf.train_data).stem
    conf.win_size = 10
    conf.first_cycle = 0
    conf.cycle_count = 9999999

    conf.output_path = '../experiments/' + mode + "/" + algo + \
        "/" + conf.dataset_name + "_" + str(conf.win_size) + "/"
    conf.log_file = conf.output_path + mode + "_" + algo + "_" + \
        conf.dataset_name + "_" + \
        str(episodes) + "_" + str(conf.win_size) + "_log.txt"

test_data_loader = TestCaseExecutionDataLoader(conf.train_data, dataset_type)
...
experiment(mode=mode, algo=algo.upper(), test_case_data=ci_cycle_logs, episodes=episodes,
